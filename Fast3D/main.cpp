...
namespace Fast3d {
		...
		Object WorldToViewPort(Object WorldObject) {
			...
			for (Fast3d::Polygon WorldPolygon : WorldObject.polygons) {
				...
				for (Point WorldPoint : WorldPolygon.data) {
					...
					ViewPortPolygon.data.push_back(ViewPortPoint);
				}
		...
		void PrintPolygon(Object WorldObject, Camera Cam) {
			for (Polygon WorldPolygon : WorldObject.polygons) {
				if (!this->settings.RenderInvisiblePolygons) {
					...
					Vector ViewCam(this->camera.position - WorldPolygon.data[0]);
				...
				for (int i = 0; i < BPS; i++) {
					Point ScreenPoint1 = BufferPolygon.data[i];
					Point ScreenPoint2 = BufferPolygon.data[(i + 1) % BPS];
	...
	vector<Polygon> generateSphere(float radius, int rings, int sectors) {
		vector<Polygon> spherePolys;
		...
		for (int i = 0; i < rings; ++i) {
			...
			for (int j = 0; j < sectors; ++j) {
				...
				spherePolys.push_back(Polygon(vector<Point>{
	...
	std::vector<Polygon> generateTorus(float radius, float tubeRadius, int segments, int sides) {
		std::vector<Polygon> torusPolys;
		const float PI = 3.14159265359f;
		...
		for (int i = 0; i < segments; ++i) {
			for (int j = 0; j < sides; ++j) {
				...
				torusPolys.push_back(Polygon(std::vector<Point>{
					p3, // Верх-Лево
						p0, // Низ-Лево
						p1,  // Низ-Право
						p2, // Верх-Право
...
int main() {
	vector<Fast3d::Polygon> cube = {
		Fast3d::Polygon(vector<Point> {Point(-0.5f, -0.5f, -0.5f), Point(-0.5f,  0.5f, -0.5f), Point( 0.5f,  0.5f, -0.5f), Point( 0.5f, -0.5f, -0.5f)}), //front
		Fast3d::Polygon(vector<Point> {Point( 0.5f,  0.5f, -0.5f), Point( 0.5f,  0.5f,  0.5f), Point( 0.5f, -0.5f,  0.5f), Point( 0.5f, -0.5f, -0.5f)}), //right
		Fast3d::Polygon(vector<Point> {Point(-0.5f, -0.5f,  0.5f), Point( 0.5f, -0.5f,  0.5f), Point( 0.5f,  0.5f,  0.5f), Point(-0.5f,  0.5f,  0.5f)}), //back
		Fast3d::Polygon(vector<Point> {Point(-0.5f,  0.5f, -0.5f), Point(-0.5f, -0.5f, -0.5f), Point(-0.5f, -0.5f,  0.5f), Point(-0.5f,  0.5f,  0.5f)}), //left
		Fast3d::Polygon(vector<Point> {Point(-0.5f,  0.5f,  0.5f), Point( 0.5f,  0.5f,  0.5f), Point( 0.5f,  0.5f, -0.5f), Point(-0.5f,  0.5f, -0.5f)}), //top
		Fast3d::Polygon(vector<Point> {Point(-0.5f, -0.5f, -0.5f), Point( 0.5f, -0.5f, -0.5f), Point( 0.5f, -0.5f,  0.5f), Point(-0.5f, -0.5f,  0.5f)}), //bottom
	};
	Object Cube(cube);
